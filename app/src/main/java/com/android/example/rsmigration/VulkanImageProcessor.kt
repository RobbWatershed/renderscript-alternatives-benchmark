...
class VulkanImageProcessor(context: Context) : ImageProcessor {
    ...
    override fun rotateHue(radian: Float, outputIndex: Int): Bitmap {
        Log.i(
            "processorK",
            "rotateHue radian = $radian; outputIndex = $outputIndex"
        )
    ...
    override fun blur(radius: Float, outputIndex: Int): Bitmap {
        Log.i(
            "processorK",
            "blur radius = $radius; outputIndex = $outputIndex"
        )
    ...
    override fun resize(percent: Float, outputIndex: Int): Bitmap {
        Log.i(
            "processorK",
            "resize percent = $percent; outputIndex = $outputIndex"
        )
        val success = resize(mVulkanProcessor, percent, 0f, outputIndex) // 0=bicubic; 1=Lanczos
        if (!success) throw RuntimeException("Failed to resize")
        return mOutputImages[outputIndex]
    }
